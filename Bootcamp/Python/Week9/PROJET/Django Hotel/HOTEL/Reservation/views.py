...
from Connexion.models import Customer
...
def contact(request):
    if request.method=="GET":
     ...
     data.save()
     return render(request,"contact/contact.html",{'message':'Merci de nous contacter.'})
def book(request):
...
def book_confirm(request):
    ...
    messages.info(request,"La chambre a été réservée avec succès")
...
def cancel_room(request,id):
    ...
    room.save()
    data.delete()
    return HttpResponse("Réservation annulée avec succès")
def delete_room(request,id):
    data=Rooms.objects.get(id=id)
    manager=data.manager.username
    if manager==request.session['username']:
        data.delete()
        return HttpResponse("Vous avez supprimé la salle avec succès")
    else:
